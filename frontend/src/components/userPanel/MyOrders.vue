    ...
    <template v-slot:default>
      ...
      <tbody>
        <tr v-for="(item, index) in orders" :key="index">
          ...
          <td>
            <v-btn text class="info darken-1" :to="`/myorder/${item._id}`">
            View
            </v-btn>
          </td>
          <td>
            <v-btn text class="error darken-1"
              v-show="item.deliveryStatus === 'dispatched'"
...
export default {
  ...
  async created () {
    this.orders = await this.$store.dispatch('getMine')
